...
class NER_AND_AL_Pipeline:
    ...
    def __init__(
        self,
        training_data_path="db/annotations/combined_annotation_launchlab_ai4g.csv",
        ...
        self.x_train, self.x_val, self.y_train, self.y_val = train_test_split(
            self.training_dtf["text"],
            self.training_dtf["biluo"],
            random_state=42,
            stratify=self.training_dtf["has_entity"],
            test_size=0.15,
        )
        print(
            "Training set size: ",
    ...
    def word_embedding(self):
        ...
        self.labels = [
            ...
            if label not in ["O", "[PAD]", "[CLS]", "[SEP]", "X"]
        ]
        ...
        self.training_text_vec = list(
            self.x_train.apply(
                lambda x: sentences_to_vec(
                    x, d_word_id=self.d_word_id, max_len=self.max_len
                )
            ).values
        )
        self.training_tags_vec = list(
            self.y_train.apply(
                lambda x: tags_to_vec(ast.literal_eval(x), d_tags_id=self.d_tags_id)
            ).values
        )

        self.training_text_vec_nopad = list(
            self.x_train.apply(
                lambda x: sentences_to_vec_nopad(
                    x, d_word_id=self.d_word_id, max_len=self.max_len
                )
            ).values
        )
        self.training_tags_vec_nopad = list(
            self.y_train.apply(
                lambda x: tags_to_vec_nopad(
                    ast.literal_eval(x), d_tags_id=self.d_tags_id
                )
            ).values
        )
        ...
        non_aligned = [
            i
            for i in range(len(self.training_text_vec_nopad))
            if len(self.training_text_vec_nopad[i])
            != len(self.training_tags_vec_nopad[i])
        ]
        ...
        self.word_embeds = np.random.uniform(
            -np.sqrt(0.06), np.sqrt(0.06), (len(self.preprocessor.vocab), embedding_dim)
        )
    ...
    def build_bilstm_crf(self):
        ...
        train_dl = DataLoader(
            TensorDataset(train_xs, train_ys), batch_size, shuffle=True
        )
    ...
    def predict_eval(self, ret_all_metrics=False):
        ...
        self.val_tags_vec = list(
            self.y_val.apply(
                lambda x: tags_to_vec(ast.literal_eval(x), d_tags_id=self.d_tags_id)
            ).values
        )

        self.val_text_vec_nopad = list(
            self.x_val.apply(
                lambda x: sentences_to_vec_nopad(
                    x, d_word_id=self.d_word_id, max_len=self.max_len
                )
        ...
        non_aligned = [
            i
            for i in range(len(self.val_text_vec_nopad))
            if len(self.val_text_vec_nopad[i]) != len(self.val_tags_vec_nopad[i])
        ]
        ...
        non_aligned = [
            i
            for i in range(len(self.tag_seq_))
            if len(self.tag_seq_[i]) != len(self.y_val_[i])
        ]
        ind_keep = [el for el in range(len(self.val_text_vec)) if el not in non_aligned]
        ...
        if entity_return_report:
            entity_f1_score, entity_return_report = eval.entity_level_f1(
                self.y_val_,
                self.tag_seq_,
                entity_digits,
            ...
            entity_f1_score = eval.entity_level_f1(
                self.y_val_,
                self.tag_seq_,
                entity_digits,
                entity_return_report,
        ...
        if ret_all_metrics:
            precision, recall, f_score, support = eval.detailed_metrics(
                self.y_val_, self.tag_seq_, average=entity_average
            )
    ...
    def eval(self, unannotated_texts, unannotated_labels):
        ...
        self.unannotated_texts_vec = list(
            unannotated_texts.apply(
                lambda x: sentences_to_vec(
                    x, d_word_id=self.d_word_id, max_len=self.max_len
                )
            ).values
        ...
        eval_dl = DataLoader(
            TensorDataset(unannotated_texts, unannotated_labels), 256, shuffle=False
        )
        self.model.eval()
    ...
    def active_learning(
        ...
        idx = strategy.select_idx(
            choices_number=choice_number, probs=probs, scores=scores, best_path=tag_seq
        )
